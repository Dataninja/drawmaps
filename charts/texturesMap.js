(function() {
    ...
    var chart = raw.chart()
        ...
        .thumbnail("imgs/texturesMap.png")
        .category("Map")
        .model(model);
    ...
    function draw(selection, data, map) {
        ...
        var pathScale = d3.scale.ordinal()
            .domain(d3.map(Object.values(data), function(d) { return d; }).keys())
            .range(["squares","nylon","waves","woven","crosses","caps","hexagons"]);
        ...
        g.selectAll("path")
            ...
            .attr("fill", function(d) {
                var t = textures.paths()
                    .strokeWidth(1)
                    .size(sz)
                    .d(pathScale(d.properties.value = data[d.properties.id]));
                svg.call(t);
                return t.url();
            })
            .attr("d", path)
